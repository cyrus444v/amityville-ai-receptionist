...
export function buildTenant(options) {
  ...
  return {
    ...
    // Defaulted here, alongside the timezone, and required by the schema: the
    // scaffolder is a starting point an operator reads and edits, so a clinic
    // outside the NANP changes one line. What the schema will not do is let a
    // *deployed* clinic omit it — see the note in src/config/tenant.ts.
    default_phone_region: options.phoneRegion ?? 'US',
...
export function buildEnvironment(options, environment) {
  ...
  return {
    ...
    description: isProduction
      ...
      : `Isolated rehearsal environment for ${tenant.display_name}. Must never reference the live calendar, the live appointment spreadsheet, or the live voice agent.`,
    ...
    values: {
      ...
      TELEPHONY_CALLER_PHONE_HEADER: 'x-caller-phone',
      TELEPHONY_CALL_ID_HEADER: 'x-call-id',
...
export function remainingSteps(tenant) {
  return [
    ...
    `Store this clinic's operator-supplied secrets: GOOGLE_CREDENTIALS_BASE64, RESEND_API_KEY.`,
    `node infra/render.mjs --tenant ${tenant.slug} --env staging --image-tag <sha>`,
    `node scripts/render-agent.mjs --tenant ${tenant.slug} --env staging`,
    'Point the voice frontend at the generated tools and prompt, and place a test call.',
  ];
}
