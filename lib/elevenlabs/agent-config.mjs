...
/**
 * A transfer destination as ElevenLabs will accept it: digits, with at most a
 * leading `+`. Tenant files carry the clinic's number the way a human reads it
 * aloud — "+1 631-691-0200" — and that spelling is right for the prompt and
 * wrong for this field: agents/{id} rejects it with "Phone number can only
 * contain alphanumeric characters, plus signs (+), and asterisks (*)" and the
 * agent cannot be published at all.
 *
 * Returns null when nothing usable is left, so the transfer tool is omitted
 * rather than declared with a destination that will not dial.
 */
export function transferDestination(raw) {
...
export function builtInTools(tenant) {
  const transferNumber = transferDestination(
...
export function buildConversationConfig(tenant, prompt, options = {}) {
  ...
  return {
    asr: {
      quality: 'high',
      // 'elevenlabs' (Original ASR) was removed by the vendor; agents/{id}
      // rejects it with original_asr_removed (400, 4 September 2026) and
      // existing agents were migrated to Scribe v2 Realtime automatically.
