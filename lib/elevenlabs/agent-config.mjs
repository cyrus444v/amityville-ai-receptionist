 ...
 *
 * The 278-line reviewed system prompt is re-homed here, not rewritten: it is
 ...
 */
/**
 * Normalises a dialling string to what the ElevenLabs transfer destination
 * accepts: digits, a leading `+`, and `*` for post-dial sequences.
 *
 * Tenant files carry the clinic's number in the form a human reads aloud —
 * `+1 631-691-0200` — because the same field feeds the prompt, the email
 * footer and the website. That form is correct there and rejected here: the
 * agent save fails validation with "Phone number can only contain
 * alphanumeric characters, plus signs (+), and asterisks (*)", which takes
 * the whole agent down, not just the transfer.
 *
 * Normalising at the boundary keeps both true — the tenant file stays
 * readable, and no clinic can break its own agent by formatting a number the
 * way every other system expects it.
 */
export function toDiallableNumber(raw) {
  if (typeof raw !== 'string') return undefined;

  const trimmed = raw.trim();
  if (!trimmed) return undefined;

  // `00` is the international prefix everywhere outside NANP; the API wants `+`.
  const withPlus = trimmed.startsWith('00') ? `+${trimmed.slice(2)}` : trimmed;
  const leadingPlus = withPlus.startsWith('+');

  // Everything else — spaces, dots, hyphens, parentheses, slashes — is
  // presentation and is dropped.
  const body = withPlus.replace(/[^0-9*]/g, '');
  if (!body) return undefined;

  return leadingPlus ? `+${body}` : body;
}

export function builtInTools(tenant) {
  const transferNumber = toDiallableNumber(
    tenant.voice?.transfer_number ?? tenant.contact?.phone,
  );
  const spoken = tenant.prompt?.spoken_name ?? tenant.display_name;
  ...
  return {
    ...
    agent: {
      ...
      prompt: {
        ...
        temperature: 0,
        // The clinic reviewed 278 lines of instructions. ElevenLabs' stock
 ...
 *
 * The 278 reviewed lines are not touched. This adds a delimited block after
 ...
 *
 * The wording is careful about one thing. The reviewed prompt's TOOL RULES say
 * "never guess the date without calling get_current_date first", and an
 ...
 * current for this call. Nothing is contradicted, and the tool remains
 * authoritative for anything further out than tomorrow.
 */
...
These are the same values get_current_date returns, already current for this call, so today's and tomorrow's dates are known and are not a guess. Call get_current_date when you need a date further out than tomorrow.
`;
}
