 ...
 *
 *  1. An unrecognised `{{placeholder}}` in a tool header throws. agent/tools.json
 *     writes headers in its own placeholder vocabulary; ElevenLabs resolves a
 *     different set, so every one has to be mapped explicitly. A placeholder
...
/** Tool-file placeholder -> the ElevenLabs system dynamic variable that replaces it. */
