...
| `RETELL_WEBHOOK_SECRET` | Yes | Yes | Retell webhook/API key used to verify `X-Retell-Signature` against the exact raw body. Production requires at least 32 characters. |
| `RETELL_WEBHOOK_TOLERANCE_MS` | No | No | `300000`. Maximum signature timestamp skew. |
| `RETELL_CALLER_PHONE_HEADER` | No | No | `x-retell-caller-phone`. Trusted dynamic header that Retell must set to `{{user_number}}` for lookup, reschedule, and cancel tools. |
| `RETELL_CALL_ID_HEADER` | No | No | `x-retell-call-id`. Trusted dynamic header that Retell should set to `{{call_id}}` for per-call shared rate limiting. |
...
| `ALLOWED_ORIGINS` | No | No | Empty. Comma-separated browser origins; requests without an `Origin` header remain allowed for Retell server-to-server calls. Avoid `*` in production. |
...
| `BUSINESS_NAME` | No | No | Clinic display name. |
...
`aws-setup.sh` additionally requires `GOOGLE_CREDENTIALS_BASE64_FILE`, `TOOL_AUTH_SECRET_FILE`, `APPOINTMENT_TOKEN_SECRET_FILE`, and `RETELL_WEBHOOK_SECRET_FILE` to point to operator-owned readable files. It passes credential files directly to the AWS CLI and never prints their contents. `RESEND_API_KEY_FILE` is optional and handled the same way.

## Protected endpoints

Authentication applies to every Retell tool, including current date, availability, appointment lookup and writes, callbacks, service search, and deep dependency diagnostics. Send `x-tool-auth: <secret>` unless `TOOL_AUTH_HEADER` is changed. `/v1` routes and legacy aliases enforce the same policy. Only `/`, `/health`, `/clinic-info`, and `/services` remain open and non-mutating. `/retell/webhook` uses Retell's timestamped HMAC signature instead of the tool secret.

Appointment lookup also requires Retell to send the actual `{{user_number}}` in `x-retell-caller-phone`. The supplied number must match the booked number. A unique match first returns only a short-lived `appointment_token`; a second lookup call must present that token before details are disclosed. Reschedule and cancellation require the same token and re-check it against the actual caller number. Ambiguous matches disclose no appointment details and must be narrowed by original date and exact time.
...
9. Smoke-test unauthenticated `/health`, the signed `/retell/webhook` path, and authenticated read-only `/current-date` and lookup-miss calls. Do not use a write tool for the smoke test.

If authentication is misconfigured, fix the Retell and ECS secret references before redeploying. Production intentionally refuses to start without `TOOL_AUTH_SECRET`; do not weaken that check as a rollback.
