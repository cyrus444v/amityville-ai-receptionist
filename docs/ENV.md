...
Numbers are matched in E.164 (`+491742306370`), and that is the only form a lookup key is ever in. Every phone number — the one a caller reads out, the caller ID in `x-caller-phone`, the string already sitting in the table — goes through the same normalisation before any comparison, so `00491742306370`, `+491742306370` and the national `0174 2306370` are one key rather than three. A number given without a country code is read in the clinic's own region, which is `default_phone_region` in `tenants/<slug>.json`: a required ISO 3166-1 alpha-2 code, `US` for both clinics in this repository. It has no default on purpose — a clinic booted under the wrong region stores and looks up the same patient under two different keys and simply never finds the appointment.
